...
public class doctor_Dao {
	...
	public int register(String name,int did,String timing,String info)
	{
		...
		try{
			...
			PreparedStatement ps=con.prepareStatement("insert into doctor (name, did, timing, info) values(?, ?, ?, ?)");
			ps.setString(1,name);
			ps.setInt(2,did);
	...
	public int update(int did,String timing)
	{
		int i=0;
		try{
			Connection con=connect();
			
			PreparedStatement ps=con.prepareStatement("update doctor set timing=? where did=?");
